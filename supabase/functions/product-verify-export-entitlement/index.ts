...
import { ensureProfile, getProfileByUserId, getUserFromRequest, publicProfile, supabaseRest } from "../_shared/product-supabase.ts";
...
async function getUsageRow(userId: string, usageDate: string, productSlug: string) {
  const rows = await supabaseRest<Record<string, unknown>[]>(
    `product_export_usage_daily?user_id=eq.${encodeURIComponent(userId)}&product_slug=eq.${encodeURIComponent(productSlug)}&usage_date=eq.${usageDate}&select=*&limit=1`
  );
  return rows?.[0] || null;
}

async function consumeUsage(userId: string, usageDate: string, requestedCount: number, dailyLimit: number, productSlug: string) {
  return await supabaseRest<Record<string, unknown>>("rpc/consume_product_export_usage_daily", {
    method: "POST",
    body: {
      p_user_id: userId,
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const profile = await getProfileByUserId(user.id, productSlug) || {};
    const plan = String(profile.plan || "free");
    const isPro = plan === "pro";
    const dailyLimit = getDailyLimit(profile);

    const usageRow = await getUsageRow(user.id, usageDate, productSlug);
    ...
    if (allowed && consume && !isPro) {
      const consumedUsage = normalizeConsumedUsage(
        await consumeUsage(user.id, usageDate, requestedCount, dailyLimit, productSlug),
        usageDate
      );
