...
import { getUserFromRequest, supabaseRest } from "../_shared/supabase.ts";
...
function sanitizeEvent(value: unknown, guestId: string, userId: string | null) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return null;
  }
...
async function consumeRateLimit(bucketKey: string, eventCount: number, limit: number) {
...
async function consumeAnalyticsRateLimit(request: Request, guestId: string, eventCount: number) {
  const minuteBucket = Math.floor(Date.now() / 60000);
  const guestBucketKey = `${PRODUCT_SLUG}:analytics:guest:${guestId}:${minuteBucket}`;
  const sourceBucketKey = `${PRODUCT_SLUG}:analytics:source:${await getSourceFingerprint(request)}:${minuteBucket}`;

  const guestAllowed = await consumeRateLimit(guestBucketKey, eventCount, MAX_EVENTS_PER_MINUTE_PER_GUEST);
  if (!guestAllowed) {
    return false;
  }

  return await consumeRateLimit(sourceBucketKey, eventCount, MAX_EVENTS_PER_MINUTE_PER_SOURCE);
}
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const rows = events
      .slice(0, 25)
      .map((event) => sanitizeEvent(event, guestId, userId))
      .filter((event): event is Record<string, unknown> => Boolean(event));

    if (rows.length > 0) {
      const allowed = await consumeAnalyticsRateLimit(request, guestId, rows.length);
      if (!allowed) {
