import { PRODUCT_SLUG } from "./plans.ts";
...
export async function ensureProfile(user: AuthUser, updates: Record<string, unknown> = {}) {
  const rows = await supabaseRest<Record<string, unknown>[]>("profiles?on_conflict=id", {
    method: "POST",
...
export function publicProfile(profile: Record<string, unknown> | null | undefined, user?: AuthUser) {
  ...
  return {
    id: stringOrDefault(source.id, user?.id || ""),
    email: stringOrDefault(source.email, user?.email || ""),
    product_slug: stringOrDefault(source.product_slug, PRODUCT_SLUG),
